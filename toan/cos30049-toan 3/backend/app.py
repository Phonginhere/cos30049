...
def read_countries_from_csv(iso3_code, CSV_FILE_PATH):
...
def validate_data(input_data: ValidationInput):
    ...
    if pollutant_data.empty:
        errors.append(f"No data found for the specified pollutant: {pollutant} in country: {read_countries_from_csv(iso3, CSV_FILE_PATH)}.")

    # If there are any errors, return them all
    if errors:
        raise HTTPException(status_code=400, detail=errors) #return {"prediction": result}

    # if errors:
    #     return {"errors": "oh hello", "status_code": 400}
